<?php       
    ...
    $conn = connect_db(); 
        ...
        $query = "insert into announce(content, grade) values('[검차+운영위]$data->push_text', '2')";
        $tokens = array();
        if( $result = mysqli_query($conn, $query) )
        {        
            echo json_encode( array('packet_id' => $data->packet_id, 'result' => 'true') );
            //$result = mysqli_query($conn,$query);
            $query = "select * from member where grade = '".$data->grade_in."' or grade = '".$data->grade_co."'";
            $result = mysqli_query($conn,$query);
        ...
        $title = "AARK 사례집";
        $message = "[검차+운영위]$data->push_text";
        $channel_id = "notice";
        //$clickAction = "annonceFragment"
