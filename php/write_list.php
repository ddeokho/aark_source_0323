<?php
    ...
    $query = "select * from case_text where member_seq = '$data->member_seq' order by TIMESTAMP desc"; //desc   
    ...
    {  
        ...
        $query2 = "SELECT case_text.seq, case_text.member_seq, case_text.year, case_text.title, case_text.content, case_text.cate_reg, case_text.cate_1, case_text.cate_2, case_text.cate_3, case_text.feq, case_text.grade, case_text.img_1, case_text.img_2, case_text.img_3, case_text.timestamp, case_text.comment_count, case_text.comment_timestamp FROM case_text LEFT JOIN comment ON case_text.seq = comment.example_seq WHERE comment.member_seq= '$data->member_seq' order by TIMESTAMP desc";//
            ...
            if( $result2 = mysqli_query($conn, $query2) )
            {
              while($row2 = mysqli_fetch_array($result2)) 
              {
                array_push($data_list, array(
                    ...
                    "comment_timestamp" => $row2['comment_timestamp']
                ));
            }
        }
